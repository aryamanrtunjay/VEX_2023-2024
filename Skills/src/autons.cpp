...
void AllAutons() {
  ...
  pros::Task::delay(350);
  chassis.pid_drive_set(-16_in, DRIVE_SPEED/2);
  pros::Task::delay(500);
  chassis.pid_turn_set(-16_deg, TURN_SPEED);
  pros::Task::delay(400);
  chassis.pid_drive_set(-4_in, DRIVE_SPEED/2, false);
  double targetCata = 0;
  double time = 0;
  while(time <= 12000) {
    left_cata.move(89);
  ...
  right_cata.move(0);
  pros::Task::delay(380);
  chassis.pid_swing_set(ez::LEFT_SWING, 45, SWING_SPEED, -SWING_SPEED * 0.30);
  pros::Task::delay(600);
